...
def post_cnn_onset(pitches,onset_frame):
	real_onset_frame = []
	for i,frame in enumerate(onset_frame):
		while frame< len(pitches) and pitches[frame]<=25:
			frame+=1 
		if i<len(onset_frame)-1 and abs(frame - onset_frame[i+1])<=8:
			continue
			
		pitch = pitches[frame:frame+8]
		idx = np.where(pitch<25)[0]
		if len(idx)<=2:		
			real_onset_frame.append(frame)
    ...
    for number in range(linenumber):
        ...
        for note_info in noteList:
            if int(note_info['pitch']) != 0:
                score_pitches.append(int(note_info['pitch']))   #有效音符列表
                pitchesLoc.append(count)    #有效音符列表位置
                type_key = float(note_info['type'])     #类型？
                if type_key in cfg.note_type_param.keys():
                    note_type = cfg.note_type_param[float(note_info['type'])]
                else:
                    note_type = -1
                note_types.append(note_type)    #音符类型
            else:
                pauseLoc.append(count)  #休止符位置列表？
            count += 1
...
def get_wav_and_json_file(root_path, wav_files, score_json):
    '''
            获取文件夹下的音频和乐谱文件
    '''
    path_list = [os.path.join(root_path, file)
...
def save_files(wav_file, onset_time, pitch, det_note, score_note, final_onset):
    '''
    ...
    final_onset_file = os.path.splitext(wav_file)[0] + "_final_onset.txt"

    final_onset_time = np.array(final_onset) * \
        cfg.post_process_param['hopsize_t']
    ...
    save_array(final_onset_file, final_onset_time)


def draw_array(arrays):
