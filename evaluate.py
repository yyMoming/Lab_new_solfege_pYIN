...
def cal_real_acc(ground_notes, score_notes):
    acc_res = dict()
    for fid in ground_notes.keys():
        temp = dict()
        gnote = np.array(ground_notes[fid])
        snote = np.array(score_notes[fid])
        gnote = is_octive(gnote, snote)
        diff_note = abs(gnote - snote)
...
def cal_ground_system(ground_acc_res, sys_acc_res):
    sys_ground_acc_res = dict()
    for fid in ground_acc_res:
        temp = dict()
        gnote = ground_acc_res[fid]['gnote']
        dnote = sys_acc_res[fid]['dnote']
        diff_note = abs(gnote - dnote)
        acc_index = np.where(diff_note <= 0.8)[0]
        accuracy = len(acc_index) / len(diff_note)
        temp['acc'] = accuracy
        temp['gnote'] = gnote
        temp['dnote'] = dnote
        temp['diff_note'] = diff_note
        sys_ground_acc_res[fid] = temp

    return sys_ground_acc_res


def visualization_save_acc(ground_acc_res, sys_acc_res, sys_ground_acc_res):
    ...
    for fid in ground_acc_res:
        ...
        print(strs)
        strs = '{:.3f}\t{:.3f}\t{:.3f}\n'.format(ground_acc_res[fid]['acc'], sys_acc_res[
                                                 fid]['acc'], sys_ground_acc_res[fid]['acc'])
        print(strs)

        file_name = os.path.join('log', fid + '.txt')
        with open(file_name, 'w') as fw:
            fw.write('gnote\t\tsnote\t\tdnote\n')
            for index in xrange(len(ground_acc_res[fid]['gnote'])):
                fw.write('{:.3f}\t\t{:.3f}\t\t{:.3f}\t\t'.format(ground_acc_res[
                         fid]['gnote'][index], ground_acc_res[fid]['snote'][index], sys_ground_acc_res[fid]['dnote'][index]))

                fw.write('{:.3f}\t\t{:.3f}\t\t{:.3f}\n'.format(ground_acc_res[
                         fid]['diff_note'][index], sys_acc_res[fid]['diff_note'][index], sys_ground_acc_res[fid]['diff_note'][index]))
...
if __name__ == '__main__':
    ...
    ground_acc_res = cal_real_acc(ground_notes, score_notes)
    sys_acc_res = cal_system_acc(wav_files)
    sys_ground_acc_res = cal_ground_system(ground_acc_res, sys_acc_res)

    visualization_save_acc(ground_acc_res, sys_acc_res, sys_ground_acc_res)
