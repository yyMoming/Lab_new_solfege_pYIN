...
sample_ratio = post_process_param['sample_ratio'] ##0.3
hopsize_t = post_process_param['hopsize_t'] ##0.01
...
class Evaluator(object):
	...
	def __init__(self, filename,
		...
		self.pitches = onset_offset_pitches['pitches'] ##就是算法测得的pitch
		...
		self._offset_frame = []
		self.score = self.evalutate_save_file()
	...
	def pitches_to_result(self):
		for idx,cur_onset_frame in enumerate(self.onset_frame):
			...
			if idx in self.add_zero_loc:
				...
				cur_offset_frame = self.offset_frame[idx] - 1
				pitch = self.pitches[cur_onset_frame:cur_offset_frame]
				#print("cur_onset_frame:",cur_onset_frame,"cur_offset_frame",cur_offset_frame,"pitch:",pitch)
				...
				#print(type(note),len(note),note)
					
				self.det_Note.append(round(np.mean(note),2))
	...
	def evalutate_save_file(self):
		...
		print('score:',score)
		'''
		print('name:%s---------score:%f'%(os.path.splitext(self.filename)[0],score))
		return score
